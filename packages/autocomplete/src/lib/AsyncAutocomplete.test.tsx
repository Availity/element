...
describe('AsyncAutocomplete', () => {
  ...
  test('should call loadOptions when scroll to the bottom', async () => {
    ...
    // setting baseline, no-scroll loading fallback will cause multiple calls before test height values manually set
    ...
    await waitFor(() => {
      expect(mockLoadOptions.mock.calls.length).toBe(initialCallCount);
    });

    fireEvent.scroll(listbox, { target: { scrollTop: 400 } });

    await waitFor(() => {
      expect(mockLoadOptions.mock.calls.length).toBe(initialCallCount + 1);
    });
  });
  ...
  test('should call loadOptions if more options available and not enough options for scroll', async () => {
    ...
    await waitFor(() => {
      expect(mockLoadOptions.mock.calls.length).toBeGreaterThan(1);
    });
    ...
    await waitFor(() => {
      expect(mockLoadOptions.mock.calls.length).toBe(maxCallCount);
    });
  });
