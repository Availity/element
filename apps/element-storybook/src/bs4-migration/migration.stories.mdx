...
import { BadgeExample, ChipExample, DialogExample, GridExample, ListItemExample, MenuExample, TabExample, StepperExample, SnackbarExample } from './MigrationExamples';
...
| Usage | HTML | MUI |
| ----- | ---- | --- |
| layout | `div`, `span` | <tr><td>[`Box`](./?path=/docs/components-layout-box--docs)</td><td>unstyled element, most versatile. The go-to when styling any element, i.e. pass `form` to `component` to add styles to your form.</td></tr><tr></tr><tr><td>[`Container`](./?path=/docs/components-layout-container--docs)</td><td>fixed width based on screensize, generally used on top level of app</td></tr><tr></tr><tr><td>[`Grid`](./?path=/docs/components-layout-grid--docs)</td><td>two dimensional layouts</td></tr><tr></tr><tr><td>[`Stack`](./?path=/docs/components-layout-stack--docs)</td><td>one dimensional layouts</td></tr> |<tr></tr>
| semantic/structure | headers(`h1`, `h2`, etc), `body`, `caption`, etc | <tr><td>[`Typography`](./?path=/docs/components-typography-typography--docs)</td><td>`variant` prop sets tag and styling, tag can be overwritten with `component` to keep the styling of a different variant. i.e. an `h2` with the styling of an `h5`</td></tr> |<tr></tr>
...
| `ListGroup`         | [`List`](./?path=/docs/components-list-list--docs)             | List Item, Availity `ListItemStatusCard` available | <ListItemExample/>  |
